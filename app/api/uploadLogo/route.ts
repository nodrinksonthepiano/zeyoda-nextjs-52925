...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log('📋 Logo upload details:', { artistId, fileSize: file.size, fileType: file.type });
    ...
    if (!artist.treasury_wallet || artist.treasury_wallet.toLowerCase() !== walletAddress.toLowerCase()) {
      ...
      return NextResponse.json({ 
        error: 'Permission denied: only artist treasury wallet can upload logo' 
      }, { status: 403 });
    }

    // 5. Delete old logo files first (cleanup)
    try {
      ...
      if (oldFiles && oldFiles.length > 0) {
        ...
        if (oldLogoFiles.length > 0) {
          await supabase.storage
            .from('artist-assets')
            .remove(oldLogoFiles);
          console.log('🗑️ Deleted old logo files:', oldLogoFiles);
        }
      }
    } catch (cleanupError) {
      console.warn('⚠️ Could not delete old logo files (non-critical):', cleanupError);
    }

    // 6. Upload file to Supabase Storage with unique filename
    const fileExtension = file.name.split('.').pop()?.toLowerCase() || 'png';
    const uniqueId = uuidv4();
    const fileName = `${artistId}/logo.${uniqueId}.${fileExtension}`;
