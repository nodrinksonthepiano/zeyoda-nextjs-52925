  ...
  try {
    ...
    if (!isWhitelisted && normalizedEmail) {
      if (coinPublicId) {
        ...
        if (!inviteBypassRow) {
          const { data: launchedRows, error: launchedError } = await serviceSupabase
            .from('artist_invites')
            .select('id')
            .eq('status', 'launched')
            .eq('claimed_by_email', normalizedEmail)
          ...
          if (launchedError) {
            console.error('❌ Treasure launched-email bypass lookup error:', launchedError);
          } else {
            inviteBypassRow = launchedRows?.[0] ?? null;
          }
        }
